...
GPU optimisations:
  ...
  - All YOLO models use FP16 half-precision
  - Normalisation tensors pinned on GPU
  - Crop batching for HSRP when multiple plates detected
...
class FramePipeline:
    """
    Seven-layer frame pipeline with temporal decision fusion.
    Maintains per-track state for EMA, bias, stability across frames.
    """
    ...
    def process_frame(
        ...
        # ── L3: Object Tracking ──────────────────────────────────────────
        # SimpleTracker doesn't need frame; VehicleTracker does
        if hasattr(tracker, 'tracker'):  # DeepSORT
        ...
        # With CUDA streams: HSRP forward passes can be batched / overlapped
        for v in vehicles:
            vid      = v["id"]
        ...
        # Sync HSRP stream before helmet (helmet may also need GPU)
        if USE_CUDA:
            ...
            if rider_id:
                ...
                if rider:
                    ...
                    if head is not None:
                        ...
                        helmet_decision       = self.helmet_manager.process(track_id, raw_helmet, frame_id)
                        enf["helmet"]         = {
        ...
        for v in vehicles:
            ...
            vio = self.violation_aggregator.evaluate_violation(
                ...
                hsrp_decision=hsrp_decision, helmet_decision=helmet_decision,
                ocr_decision=ocr_decision, frame_id=frame_id,
            )
            if vio["should_store"]:
