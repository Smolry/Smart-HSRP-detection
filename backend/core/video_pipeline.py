...
GPU optimisations over original:
  ...
  - Frame resize on GPU before passing to detectors
...
def process_video(
    ...
    # Use DeepSORT (GPU) when CUDA available, SimpleTracker otherwise
    try:
    ...
    # Warm-up CUDA kernels on first real frame to avoid cold-start latency
    _cuda_warmed = False

    try:
        logger.info(f"[{job_id}] Starting | gpu={USE_CUDA} | fps={source_fps}")
        ...
        for frame_id, frame in read_video(video_path):
            ...
            # GPU warm-up: run a tiny forward pass to load CUDA kernels
            ...
            try:
                ...
                for v in vehicles:
                    ...
                    pred = predictor.update(track_id=track_id, frame_id=frame_id,
                    ...
                    if plate and hsrp_score is not None:
                        learner.observe("hsrp", hsrp_score,
                                        plate.get("hsrp_confidence", 0.5), mode="semi_supervised")
                ...
                for v in vehicles:
                    ...
                    if vtype:
                        db_gate.feed(
                            ...
                            prediction_warned=(pred_info.get("any_warning") or pred_info.get("any_confirmed")),
                            prediction_risk=max(
                ...
                if output_video_path:
                    try:
                        annotated, any_pred = annotate_frame(frame.copy(), frame_result, current_predictions)
                        if any_pred:
                            annotated = enhance_frame_for_capture(annotated)

                        if video_writer is None:
                            h, w = annotated.shape[:2]
                            logger.info(f"[{job_id}] Writer: {output_video_path} {w}x{h}@{source_fps}fps")
                            video_writer = AsyncVideoWriter(output_video_path, fps=source_fps, width=w, height=h)
